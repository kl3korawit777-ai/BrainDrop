...
export interface CardItem {
  ...
  alt?: string;
  linkUrl?: string;
  /** หน้าการ์ดแบบกำหนดเอง — ถ้ามี จะแสดงแทน <img> */
  node?: ReactNode;
  /** ทำให้การ์ดเป็นปุ่มและเรียก callback เมื่อคลิก */
  onClick?: () => void;
}
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            ...
            if (card.onClick) {
              return (
                <button key={index} type="button" onClick={card.onClick} aria-label={card.alt} className="fan-card block cursor-pointer">{face}</button>
              );
            }
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer">{face}</a>
            ) : (
